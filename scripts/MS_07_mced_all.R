...
mced_all_compute_tests_per_stage<-seer_post_uspstf_incidence %>%
  left_join(mced_all_defn) %>%
  left_join(mced_all_sens) %>%
...
mced_all_compute_tests<-mced_all_compute_tests_per_stage %>%
  group_by(Cancer) %>%
  summarize(Pop=Pop[1],
            Selected=Selected[1],
...
mced_all_compute_perf_summary<-mced_all_compute_tests %>%
  filter(Selected>0) %>%
...
#cost
mced_all_compute_costs<-mced_all_compute_tests %>%
  select(Cancer,amortized_NumScreenTests,TP,amortized_FP) %>%
  cross_join(mced_all_cost %>% select(ScreeningCost)) %>%
...
mced_all_compute_cost_summary<-mced_all_compute_costs %>% 
...
write_tsv(mced_all_compute_tests,sprintf("generated_data/%s_mced_all_compute_tests.tsv",date_code))
write_tsv(mced_all_compute_costs,sprintf("generated_data/%s_mced_all_compute_costs.tsv",date_code))
